    ...
    <b-container fluid>
        <b-row style="margin-top:5%;margin-left:5%;margin-right:5%;">
            ...
            <b-col lg="6" class="my-1">
                <b-form-group
                    label="Filter On"
                    description="Uncentang untuk pencarian semua kategori"
                    label-cols-sm="3"
                    label-align-sm="right"
                    label-size="sm"
                    class="mb-0"
                ...
                >
                    <b-form-checkbox-group
                        ...
                        class="mt-1"
                    >
                        <b-form-checkbox value="key_type">Kode</b-form-checkbox>
                        <b-form-checkbox value="description">Deskripsi</b-form-checkbox>
                        <b-form-checkbox value="key_inputs">Kata Kunci</b-form-checkbox>
                        <b-form-checkbox value="value_outputs">Nilai</b-form-checkbox>
                    </b-form-checkbox-group>
                </b-form-group>
            </b-col>
        ...
        <!-- <b-row style="margin-top:5%;margin-left:5%;margin-right:5%;"> -->
            <b-navbar toggleable="lg" type="light" variant="info">
                <span class="fs-4">Transaksi Atribut ==> {{ totalRows }} Baris</span>
            </b-navbar>
        <!-- </b-row> -->
        ...
        <b-modal :id="trxModal.id" :title="trxModal.title" ok-only @hide="resetTrxModal">
            <pre>{{ trxModal.content }}</pre>
        </b-modal>
    </b-container>
</template>
<style scoped>
...
module.exports = {
    ...
    mounted(){
        console.log('mounted TrxAttr');
        this.database = [];
        this.checkMysql();
        this.retrieveData(this.date);
    ...
    methods: {
        retrieveData(date){
            axios.post(this.menu,{ jenis: 'TrxAttr', limit: true})
            .then((resp) => { 
        ...
        getTrx(trxId, tombol) {
            ...
            this.trxModal.title = trxId;
            axios.post(this.picker,{ jenis: 'Transaction', parameter: trxId})
            .then((resp) => { 
                this.trxModal.content = resp.data;
            })
            .catch(errors => { console.error(errors); });
            this.$root.$emit('bv::show::modal', this.trxModal.id, tombol);
        },
        checkMysql(){
